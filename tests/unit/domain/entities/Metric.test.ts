import type { MetricProps } from '@domain/entities/Metric';
...
describe('Metric Entity', () => {
  ...
  describe('Validation', () => {
    test('should throw error without userId', () => {
      expect(() => new Metric({ ...validMetricData, userId: '' })).toThrow('UserId is required');
    });

    test('should throw error for invalid type', () => {
      expect(() => new Metric({ ...validMetricData, type: 'invalid' as MetricType })).toThrow(
        'Type must be either "distance" or "temperature"'
      );
    });

    test('should throw error for invalid value', () => {
      expect(() => new Metric({ ...validMetricData, value: NaN })).toThrow(
        'Value must be a valid number'
      );
    });

    test('should throw error without unit', () => {
      expect(() => new Metric({ ...validMetricData, unit: '' })).toThrow('Unit is required');
    });

    test('should throw error for invalid date', () => {
      expect(() => new Metric({ ...validMetricData, date: 'invalid-date' })).toThrow(
  ...
  describe('Serialization', () => {
    test('should convert to JSON correctly', () => {
      ...
      expect(json.date).toBe('2025-12-01');
