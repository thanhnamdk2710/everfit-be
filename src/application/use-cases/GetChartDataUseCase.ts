...
import type { ChartDataDTO, ChartPointDTO, ChartDataResponseDTO } from '../dto/MetricDTO';
...
export class GetChartDataUseCase {
  ...
  async execute(dto: ChartDataDTO): Promise<ChartDataResponseDTO> {
    ...
    const chartData: ChartPointDTO[] = metrics.map((metric) => {
      ...
      const dateStr =
        metric.date instanceof Date ? metric.date.toISOString().split('T')[0] : metric.date;
